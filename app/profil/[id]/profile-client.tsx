...
import {
  ExternalLink, Edit2, Loader2, ArrowLeft,
  Tag, Calendar, CheckCircle2, Briefcase, Plus, Star, Clock, ShieldCheck, Lock, Phone, Mail,
} from "lucide-react"
...
function ZelligeCover() {
  return (
    <div className="h-52 relative overflow-hidden bg-primary shadow-md">
      <svg
...
export default function PublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Track profile view (fire and forget, skip owner self-views)
      if (profileData && user?.id !== id) {
        if (user) {
          ...
          if (!recentViews || recentViews.length === 0) {
            supabase.from("profile_views").insert({ profile_id: id, viewer_id: user.id }).then(() => {})
          }
        } else {
          supabase.from("profile_views").insert({ profile_id: id, viewer_id: null }).then(() => {})
        }
      }
    })
  }, [id])
      ...
      <>
        ...
        <main className="min-h-screen bg-background">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative z-10 -mt-16 pb-6 border-b border-border/60">
              <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
                <div className="w-32 h-32 rounded-full bg-gray-300 animate-pulse shrink-0 shadow-xl" />
                <div className="sm:pt-16 min-w-0 flex-1 pb-2 space-y-3">
                  <div className="h-8 w-48 bg-gray-200 animate-pulse rounded" />
                  <div className="h-5 w-32 bg-gray-200 animate-pulse rounded" />
                </div>
              </div>
            </div>
            <div className="py-4 sm:py-5 border-b border-border grid grid-cols-3 divide-x divide-border">
              {[0, 1, 2].map((i) => (
                <div key={i} className="text-center px-4 space-y-2">
                  <div className="h-3 w-16 bg-gray-200 animate-pulse rounded mx-auto" />
                  <div className="h-4 w-12 bg-gray-200 animate-pulse rounded mx-auto" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="py-4 sm:py-5 border-b border-border grid grid-cols-3 divide-x divide-border">
            <div className="text-center px-1 sm:px-4">
              <div className="flex items-center justify-center gap-1 sm:gap-1.5 text-muted-foreground mb-1 sm:mb-1.5">
                <Calendar className="h-3 w-3 sm:h-3.5 sm:w-3.5 shrink-0" />
                <span className="text-[10px] sm:text-xs uppercase tracking-tight sm:tracking-wide font-medium leading-tight">{t.memberSince}</span>
              </div>
              <p className="text-xs sm:text-sm font-semibold text-foreground capitalize leading-tight">
            ...
            <div className="text-center px-1 sm:px-4">
              <div className="flex items-center justify-center gap-1 sm:gap-1.5 text-muted-foreground mb-1 sm:mb-1.5">
                ...
                <span className="text-[10px] sm:text-xs uppercase tracking-tight sm:tracking-wide font-medium leading-tight">{t.avgRating}</span>
              </div>
              {avgRating !== null ? (
                <div className="flex items-center justify-center gap-1 sm:gap-1.5 flex-wrap">
                  <span className="text-xs sm:text-sm font-semibold text-foreground">{avgRating.toFixed(1)}</span>
                  <span className="hidden sm:block"><StarDisplay rating={Math.round(avgRating)} /></span>
                </div>
              ) : (
            ...
            <div className="text-center px-1 sm:px-4">
              <div className="flex items-center justify-center gap-1 sm:gap-1.5 text-muted-foreground mb-1 sm:mb-1.5">
                ...
                <span className="text-[10px] sm:text-xs uppercase tracking-tight sm:tracking-wide font-medium leading-tight">{t.missions}</span>
              </div>
          ...
          <div className="py-8 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                  ...
                  <div className="space-y-2">
                    {portfolioLinks.map((link, i) => (
                      <a
                        ...
                        className="flex items-center gap-2.5 p-3 rounded-xl border border-border/40 bg-card shadow-sm hover:border-primary/30 hover:bg-muted hover:shadow-md transition-all duration-200 group"
                      >
                ...
                {reviews.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {reviews.map((review) => {
                      ...
                      return (
                        <div key={review.id} className="rounded-2xl border border-border/40 bg-card p-4 shadow-sm">
                ...
                {services.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {services.map((svc) => (
                      <div
                        key={svc.id}
                        className="group relative rounded-2xl border border-border/40 bg-card overflow-hidden shadow-sm hover:border-primary/30 hover:shadow-lg transition-all duration-200"
                      >
                        <Link href={`/services/${svc.id}`} className="block">
                        ...
                        <FavoriteButton
                          serviceId={svc.id}
                          isFavorited={favoriteIds.has(svc.id)}
            ...
            {/* Sidebar */}
            <div>
              <div className="rounded-2xl border border-border/40 bg-card p-5 shadow-lg space-y-4 sticky top-24">
                {profile.hourly_rate != null && (
                ...
                {/* Contact info — protected unless viewer has active order */}
                {(profile.phone || profile.email) && !isOwner && (
