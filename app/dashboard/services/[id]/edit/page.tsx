...
const STEPS = [
  ...
  { id: 5, short: "Aperçu",    label: "Aperçu & Sauvegarde",  Icon: Eye        },
]
...
export default function EditServicePage({
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (svc.user_id !== data.user.id) {
        router.replace("/dashboard")
        return
      }

      const normalizeTier = (t: unknown): Tier => {
        const tier = t as Record<string, unknown> | null
        return {
      ...
      const rawTiers = svc.pricing_tiers as Record<string, unknown> | null
      const tiers = {
        basic:    normalizeTier(rawTiers?.basic ?? { name: "Basique",  price: String(svc.price ?? ""), delivery_days: String(svc.delivery_days ?? "") }),
        standard: normalizeTier(rawTiers?.standard),
        premium:  normalizeTier(rawTiers?.premium),
      }
  ...
  function ingest(files: FileList | null) {
    if (!files) return
    const slots = MAX_IMGS - totalImages
    Array.from(files).slice(0, slots).forEach(f => {
      ...
      reader.onload = ev => {
        ...
        setDraft(d => ({
          ...d,
          newImageDataUrls: [...d.newImageDataUrls, url].slice(
            0, MAX_IMGS - d.existingImageUrls.length
          ),
        }))
      }
  ...
  function removeExistingImg(i: number) {
    patch({ existingImageUrls: draft.existingImageUrls.filter((_, j) => j !== i) })
  }
  ...
  async function save() {
    ...
    for (const dataUrl of draft.newImageDataUrls) {
      try {
  ...
  const previewImage = draft.existingImageUrls[0] ?? draft.newImageDataUrls[0]
      ...
      <div className="min-h-screen bg-[#FAFAF8] py-10 px-4">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mb-8 animate-fade-in-up animation-delay-100">
            <h1 className="text-3xl font-bold tracking-tight text-gray-900">Modifier le service</h1>
          ...
          <div key={step} className="animate-fade-in-up">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Gérez les images de votre service. Maximum {MAX_IMGS} images au total.
                  </p>
                </div>
                ...
                <div
                  ...
                  onClick={() => totalImages < MAX_IMGS && fileRef.current?.click()}
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      JPG, PNG, WEBP · Max 5 Mo par image · {totalImages}/{MAX_IMGS} images
                    </p>
                  </div>
                  <input
                    ref={fileRef}
                ...
                {totalImages > 0 && (
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    {draft.existingImageUrls.map((url, i) => (
                      <div key={`ex-${i}`} className="relative group aspect-square rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-150 flex items-center justify-center">
                          <button
                            onClick={e => { e.stopPropagation(); removeExistingImg(i) }}
                    ...
                    {draft.newImageDataUrls.map((url, i) => (
                      <div key={`nw-${i}`} className="relative group aspect-square rounded-xl overflow-hidden border border-blue-200 bg-gray-50">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={url} alt="" className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-150 flex items-center justify-center">
                          <button
                            onClick={e => { e.stopPropagation(); removeNewImg(i) }}
                            className="opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 rounded-full bg-white shadow-md flex items-center justify-center text-red-600 hover:bg-red-50"
                            aria-label="Supprimer cette image"
                          >
                            <X className="h-4 w-4" />
                        ...
                        {draft.existingImageUrls.length === 0 && i === 0 && (
                          <span className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-600 text-white leading-none">
                            Principale
                          </span>
                        )}
                        <span className="absolute bottom-1.5 right-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-600 text-white leading-none">
                          Nouveau
                        </span>
                      </div>
                    ))}
            ...
            {/* ════ STEP 5 : Aperçu & Sauvegarde ════ */}
            {step === 5 && (
              <div className="space-y-5">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900 mb-1">Aperçu & Sauvegarde</h2>
                  <p className="text-sm text-gray-500">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                  {previewImage ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={previewImage} alt="Aperçu du service" className="w-full h-60 object-cover" />
                  ) : (
                  ...
                  <div className="p-6">
                    ...
                    {totalImages > 1 && (
                      <div className="border-t border-gray-100 pt-4 mt-4 flex gap-2 overflow-x-auto pb-1">
                        {[...draft.existingImageUrls, ...draft.newImageDataUrls].map((url, i) => (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img
                            key={i}
                            src={url}
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5">
                  <p className="text-sm font-semibold text-gray-900 mb-3">Récapitulatif des modifications</p>
                  <div className="space-y-2.5">
                    {[
                      { label: "Titre",       ok: draft.title.trim().length >= 10,                                                                     val: draft.title || "—" },
                      { label: "Catégorie",   ok: !!draft.subcategory,                                                                                 val: draft.subcategory ? getCategoryLabel(draft.subcategory) : "—" },
                      { label: "Description", ok: draft.description.trim().length >= 50,                                                               val: `${draft.description.trim().length} car.` },
                      { label: "Tarification",ok: !!(draft.tiers.basic.price && draft.tiers.standard.price && draft.tiers.premium.price),              val: `${draft.tiers.basic.price || "—"} / ${draft.tiers.standard.price || "—"} / ${draft.tiers.premium.price || "—"} DH` },
                      { label: "Médias",      ok: totalImages > 0,                                                                                     val: `${totalImages} image(s)` },
                      { label: "Exigences",   ok: true,                                                                                                val: `${draft.requirements.filter(r => r.trim()).length} question(s)` },
                    ].map(({ label, ok, val }) => (
...
function TierField({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
